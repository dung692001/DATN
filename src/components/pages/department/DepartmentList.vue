...
export default {
    ...
    methods: {
        ...
        async loadDataByFilter() {
            ...
            try {
                ...
                await DepartmentApi.loadDataByFilter(this.recordNumber, this.currentPage, currentWindow.currentSearchValue).then(
                    (res) => {
                        currentWindow.totalPage = res.data.TotalPage;
                        currentWindow.departments = res.data.Data;
        ...
        tickAllCheckBox() {
            try {
                ...
                let maxNumberInPage = (this.currentPage - 1) * this.recordNumber + this.departments.length;
                let j = 0;
                if (this.countCheckBoxCheckedInPage != this.departments.length) {
                    for (i; i < maxNumberInPage; i++, j++) {
                        if (this.arrayCheckBoxSelectedId[i] != null) {
                            this.countIdInArray = this.countIdInArray - 1;
                        }
                        this.countIdInArray = this.countIdInArray + 1;
                        this.arrayCheckBoxSelectedId[i] = this.departments[j].DepartmentId;
                    }
                    this.countCheckBoxCheckedInPage = this.departments.length;
                } else {
         ...
         */
        prepareDeleteMultidDepartments() {
            this.showWarningPopup(true, resourceVN.DeleteMultidepartment, deleteMode.multi);
        },
        ...
        btnExportExcel() {
            try {
                DepartmentApi.exportdepartment().then(
                    () => {},
                    (err) => {
                        console.log(err);
                    }
        ...
        toggleRowSelected(depatmentId) {
            if (this.departmentRowId == depatmentId) {
                return true;
            } else {
                return false;
            }
        ...
        selectedRow(depatmentId) {
            if (this.departmentRowId == depatmentId) {
                this.departmentRowId = '';
            } else {
                this.departmentRowId = depatmentId;
            }
        },
        ...
        btnAddOnClick() {
            ...
            this.showDepartmentDetailDialog(true);
            this.formMode = eventMode.add;
        },

        /**
         * Hiển thị form chi tiết nhân viên khi ấn nút sửa tại dòng dữ liệu
         * @param department thông tin nhân viên
         * @Author NDDung (23/07/2022)
         */
        btnEditOnClick(department) {
            this.showDepartmentDetailDialog(true);
            this.departmentSelectedId = department.DepartmentId;
            this.departmentContextMenuId = '';
            this.formMode = eventMode.edit;
        },

        /**
         * Hiển thị viền bao quanh nút context menu
         * @param departmentId Khóa chính của nhân viên
         * @Author NDDung (25/07/2022)
         */
        btnContextMenuOnClick(depatmentId) {
            if (this.departmentContextMenuId == depatmentId) {
                this.departmentContextMenuId = '';
            } else {
                this.departmentContextMenuId = depatmentId;
            }
        },
         ...
         * @Author NDDung (25/07/2022)
         */
        checkShowContextMenu(depatmentId) {
            if (this.departmentContextMenuId == depatmentId) {
                return true;
            } else {
                return false;
         ...
         * @Author NDDung (03/08/2022)
         */
        btnCloneOnClick(depatment) {
            this.showDepartmentDetailDialog(true);
            this.depatmentSelectedId = depatment.DepatmentId;
            this.departmentContextMenuId = '';
         ...
         * @Author NDDung (23/07/2022)
         */
        showDepartmentDetailDialog(isShowDialog) {
        ...
        async confirmDelete() {
            ...
            if (this.deleteStatus == 0) {
                ...
                try {
                    await DepartmentApi.deleteDepartment(currentWindow.departmentContextMenuId).then(
                        (res) => {
                            res;
                            this.loadDataByFilter();
    ...
    data() {
        return {
            ...
            departmentContextMenuId: null,
            /** Biến lưu id nhân viên khi tick vào checkbox*/
            departmentIdCheckboxSelected: null,
            ...
            totalRecord: null,
            /** Biến kiểu bool để show DepartmentDetail*/
            isShowDialog: false,
...
<template>
    <div class="content__header">
        ...
        <BaseButton :text="'Thêm mới phòng ban'" @click="btnAddOnClick"> </BaseButton>
    </div>
        ...
        <div class="content__main__header">
            ...
            <div class="flex">
                <div class="content__search">
                    <BaseInput v-model="searchValue" :hasIcon="true" @keyup="searchObject(searchValue)" :placeHolder="'Tìm theo mã phòng ban'" :inputIcon="'icon__search'"> </BaseInput>
                </div>
            ...
            <div class="table">
                <table id="tbDepartmentList">
                    ...
                    <tbody>
                        ...
                        <tr
                            v-for="(emp, index) in departments"
                            :key="emp"
                            :class="{
                                table__item: true,
                                'row--selected': toggleRowSelected(emp.DepartmentId),
                                'rows--checked': tickCheckBoxSelected(index, emp.DepartmentId)
                            }"
                        ...
                        >
                            ...
                            <td class="table__center">
                                <div class="table__combo__button flex">
                                    ...
                                    <div :class="['button__material btnMaterial', emp.DepartmentId == departmentContextMenuId ? 'button__material__border' : '']" @click="btnContextMenuOnClick(emp.DepartmentId)">
                                        <button class="icon icon__table__material button24"></button>
                                    </div>
                                </div>
                            </td>
                            <td class="table__popup">
                                <BaseContextMenu :class="{ isShowBlock: checkShowContextMenu(emp.DepartmentId) }" :departmentCode="emp.DepartmentCode" :department="emp" @showWarningPopup="showWarningPopup" @btnCloneOnClick="btnCloneOnClick">
                                </BaseContextMenu>
                            </td>
                        </tr>
                    </tbody>
    ...
    <DepartmentDetail
        v-if="isShowDialog"
        :departmentSelectedId="departmentSelectedId"
        :formMode="formMode"
        @showDepartmentDetailDialog="showDepartmentDetailDialog"
