...
export default {
    ...
    emits: ['showPositionsDetailDialog', 'loadDataByFilter', 'setToastMessage', 'showToastMessage', 'showToastMessage'],
    /**
    ...
    methods: {
         ...
         */
        btnCloseOnClick() {
            // showPositionsDetailDialog chỉ là cái để tham chiếu đến @showPositionsDetailDialog ở cha

            this.$emit('showPositionsDetailDialog', false);
        },
            ...
            try {
                ...
                if (currentWindow.validateInput() == true) {
                    ...
                    if (currentWindow.formMode == 0 || currentWindow.formMode == 2) {
                        ...
                        // Thực hiện thêm mới
                        PositionsApi.savePositionsByAdd(currentWindow.position).then(
                            () => {
                                ...
                                if (saveMode == 0) {
                                    // Đóng form thêm mới
                                    currentWindow.$emit('showPositionsDetailDialog', false);
                                }
                        ...
                        PositionsApi.savePositionsByEdit(currentWindow.positionSelectedId, currentWindow.position).then(
                            () => {
         ...
         */
        showPositionsDetailPopup(isShow) {
            try {
                if (isShow == false) {
        ...
        validateInput() {
            try {
                ...
                if (!this.position.PositionsCode) {
                    this.inputRequire.push('inputCode');
                    this.popupNotification.push(resourceVN.ValidateError_PositionsCodeNotEmpty);
                    valid = false;
                }
                // Kiểm tra tên có đang để trống hay không
                if (!this.position.PositionsName) {
                    this.inputRequire.push('inputName');
                    this.popupNotification.push(resourceVN.ValidateError_PositionsNameNotEmpty);
                    valid = false;
                }
        ...
        showErrorPopup() {
            ...
            this.showPositionsDetailPopup(true);
        },
        ...
        /**
         * Phương thức get position từ positionId
         * @param positionId: id truyền vào để lấy toàn bộ thông tin nhân viên
         * @Author NDDung (25/07/2022)
         */
        async getPositions(positionId) {
            let currentWindow = this;
            try {
                this.$refs['inputCode'].focusInput();
                await PositionsApi.getPositionsById(positionId).then(
                    (res) => {
                        currentWindow.position = res.data;
                        setTimeout(function () {
                            currentWindow.positionTemp = JSON.stringify(currentWindow.position);
                        }, 100);
                    },
                    (err) => {
                        console.log(err);
         ...
         */
        checkDataChange() {
            if (JSON.stringify(this.position) == JSON.stringify(this.positionTemp)) {
                this.$emit('showPositionsDetailDialog', false);
                this.position = {};
    ...
    watch: {
        positionSelectedId: function (value) {
            if (value) {
                this.getPositions(this.positionSelectedId);
            }
        },
        positionCode: function (value) {
            this.position.positionCode = value;
        }
    },
    data() {
        return {
            // object lưu 1 nhân viên được truyền vào từ positionList
            position: {},
...
<template>
    <div id="dlgPositionsDetail" class="dialog__popup" @keydown="hanldeOnKeyDown" @keyup="hanldeOnKeyUp" v-bind:class="{ isShowDialog: true }">
        <div class="dialog width-600">
            <div class="dialog__header">
                <div class="dialog__title flex">
                    <div class="title__popup">{{ formMode != 1 ? 'Thêm Chức Danh' : 'Sửa Chức Danh' }}</div>
                </div>
                    ...
                    <div class="dialog__content__up">
                        <div class="dialog__content__left">
                            <div class="dialog__row">
                                ...
                                <BaseInput ref="inputCode" tabIndex="2" :label="'Mã'" :inputRequire="true" :size="'p-r-6'" :maxLengthInput="20" :hasTooltip="true" v-model="position.PositionsCode"> </BaseInput>
                            </div>
                        </div>
                        <div class="dialog__content__right">
                            <div class="dialog__row">
                                <BaseInput ref="inputName" :label="'Tên'" tabIndex="3" :inputRequire="true" :size="''" :maxLengthInput="100" :hasTooltip="true" v-model="position.PositionsName"> </BaseInput>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <BasePopup
            ...
            @showPopup="showPositionsDetailPopup"
            @closeDetail="btnCloseOnClick"
            @savePositions="btnSaveOnClick(this.saveMode.saveDefault)"
        >
        </BasePopup>
    </div>
</template>
